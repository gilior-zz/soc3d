...
export class UserServiceProvider {
  ...
  storageControl(action: 'get' | 'set' | 'delete', key?, value?): Promise<any> {
      ...
      .then(res => {
        ...
        return this.storageControl('get', user)
          .then(userFromStorage => {
            if (!userFromStorage) {
              return this.saveNewUserToStorage(user)
                .then(res => {
                  this.success = true;
                  this.displayAlerts(user, 'user saved')
                })
            }
            else {
              this.rewardServiceProvider.rewardsCheck(user, userFromStorage)
                .then((rewardsCheckResult: UserData) => {
                  return this.updateUser(user, rewardsCheckResult)
                    .then(updatedUser => {
                      this.success = true;
                      this.displayAlerts(user, 'updated some data');
                    })
                })
